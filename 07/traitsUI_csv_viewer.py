# -*- coding: utf-8 -*-
"""
使用matplotlib控件制作的CSV绘图工具
"""
from itertools import izip
import numpy as np
from matplotlib.figure import Figure
from mpl_figure_editor import MPLFigureEditor
from enthought.traits.api import (HasTraits, DictStrAny, List, Str, 
    Instance, Button, File)
from enthought.traits.ui.api import (View, HSplit, VGroup, Heading, Item, 
    EnumEditor, CheckListEditor, ListEditor, HGroup)


class DataSource(HasTraits):
    """
    数据源，data是一个字典，将字符串映射到一维数组
    ...
    def load_csv(self, filename):
        ...
        """       
        f = file(filename)
        self.names = [x.strip() for x in f.readline().split(",")]
        array = np.loadtxt(f, delimiter=",", dtype=np.float)
        f.close()
        for name, coldata in izip(self.names, array.T):
            self.data[name] = coldata
        self.names.insert(0, "_index")
        self.data["_index"] = np.arange(array.shape[0])
