...
static PyObject* square(PyObject*self, PyObject* args, PyObject* kywds)
{
    py::object return_val;
    int exception_occured = 0;
    PyObject *py_local_dict = NULL;
    static const char *kwlist[] = {"arr","local_dict", NULL};
    PyObject *py_arr;
    int arr_used;
    py_arr = NULL;
    arr_used = 0;
    
    if(!PyArg_ParseTupleAndKeywords(args,kywds,"O|O:square",const_cast<char**>(kwlist),&py_arr, &py_local_dict))
       return NULL;
    try                              
    {                                
        py_arr = py_arr;
        PyArrayObject* arr_array = convert_to_numpy(py_arr,"arr");
        conversion_numpy_check_type(arr_array,PyArray_DOUBLE,"arr");
        conversion_numpy_check_size(arr_array,2,"arr");
        blitz::Array<double,2> arr = convert_to_blitz<double,2>(arr_array,"arr");
        blitz::TinyVector<int,2> Narr = arr.shape();
        arr_used = 1;
        /*<function call here>*/     
...
PyMODINIT_FUNC initdemo_ext(void)
{
    
    Py_Initialize();
    import_array();
    PyImport_ImportModule("numpy");
    (void) Py_InitModule("demo_ext", compiled_methods);
}
